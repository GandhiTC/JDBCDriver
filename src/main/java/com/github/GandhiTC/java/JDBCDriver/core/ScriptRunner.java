 ...
 * 
 * I have modified some error messages, the DateTime header on error logs, and some access modifiers.
 * I have commented out @SuppressWarnings("UseOfSystemOutOrSystemErr") lines.
 */
...
//	@SuppressWarnings("UseOfSystemOutOrSystemErr")
	...
	ScriptRunner(Connection connection, boolean autoCommit, boolean stopOnError)
	{
		...
		File	logFile				= new File("ScriptRunner.log");
		File	errorLogFile		= new File("ScriptRunner_error.log");
		...
		println(dateTimeHeader);
		printlnError(dateTimeHeader);
	}
	...
	private void execSqlCommand(Connection conn, StringBuffer command, LineNumberReader lineReader) throws SQLException
	{
		...
		{
			final String errText = String.format("Error executing \"%s\"\r\n(line %d): %s\r\n", command.toString().trim(), lineReader.getLineNumber(), e.getMessage());
